...
def _history_cursor_encode(fingerprint, offset, identity):
    import base64
    raw = system.util.jsonEncode({"v": 2, "f": fingerprint, "o": offset, "t": identity[0], "s": identity[1]})
...
def _history_request(payload):
    ...
    if set(payload.keys()) - set(["paths", "start", "end", "limit", "cursor"]):
...
def _capabilities(payload):
    return {"ok": True, "protocolVersion": 1, "historianStream": {
        ...
        "supported": True, "exactRawCapture": True, "input": "paths[{seriesKey,tagpath}]",
...
def dispatch(operation, payload_json):
    try:
        ...
        return system.util.jsonEncode({"status": 400, "body": {
            "ok": False, "code": "HISTORIAN_COUNT_LIMIT_EXCEEDED", "error": str(exc),
        }})
    except BadRequest, exc:
        return system.util.jsonEncode({"status": 400, "body": {"ok": False, "error": str(exc)}})
    except Conflict, exc:
        return system.util.jsonEncode({"status": 409, "body": {
            "ok": False, "code": "HISTORIAN_PAGE_UNSUPPORTED", "error": str(exc),
        }})
