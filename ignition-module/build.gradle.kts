...
val buildNumber = "20260711"
val moduleVersionValue = "$releaseVersion.$buildNumber"
val repositoryUrl = "https://github.com/GreenPipePartners/Fluxy"
...
val licenseMode = providers.gradleProperty("licenseMode").orElse("licensed")
val isIgnition81 = ignitionTarget.map { it == "8.1" || it == "8.1.50" }
...
val targetIgnitionVersion = if (isIgnition81.get()) "8.1.50" else "8.3.4"
val moduleFileName = "Fluxy-$targetSuffix$variantSuffix"
val moduleDisplayName = if (freeVariant.get()) "Fluxy Free" else "Fluxy"

require(ignitionTarget.get() in setOf("8.1", "8.1.50", "8.3", "8.3.4")) {
    "ignitionTarget must be one of: 8.1, 8.1.50, 8.3, 8.3.4"
}
...
val generateCompliance by tasks.registering {
    ...
    doLast {
        ...
        val sourceTree = if (commit.matches(Regex("[0-9a-f]{40}"))) {
            "$repositoryUrl/tree/$commit/ignition-module"
        } else {
            "$repositoryUrl/tree/main/ignition-module"
        }
        ...
        val status = if (officialRelease.get()) "OFFICIAL RELEASE" else "UNSIGNED DEVELOPMENT BUILD - NOT FOR DISTRIBUTION"
        ...
        output.resolve("SOURCE.txt").writeText(
            ...
            |
            |Preferred source is the complete ignition-module/ directory at the exact
            |commit above. Build with Java 17 and the committed Gradle wrapper:
            |
            |  cd ignition-module
            |  ./gradlew -PignitionTarget=${ignitionTarget.get()} -PlicenseMode=${licenseMode.get()} clean packageDevelopment
            |
            |Official source remains available under MPL-2.0 after maintenance ends.
...
ignitionModule {
    ...
    moduleDescription.set(
        if (freeVariant.get()) {
            "Unlicensed private-use Python 3 access to allowlisted Ignition scripting functions."
        } else {
    ...
    skipModlSigning.set(!officialRelease.get())
}
...
val verifyOfficialReleaseInputs by tasks.registering {
    doLast {
        require(officialRelease.get()) { "Official packaging requires -PofficialRelease=true" }
        require(iaVendorId > 0) { "Official packaging requires the committed IA-assigned vendorId" }
        require(freeVariant.get() || iaLicensingReady) {
            "Official licensed packaging requires completed IA licensing integration"
        }
        require(sourceCommit.get().matches(Regex("[0-9a-f]{40}"))) {
            "Official packaging requires a 40-character -PsourceCommit"
        }
        require(sourceTag.get() == "ignition-module-v$moduleVersionValue") {
            "sourceTag must be ignition-module-v$moduleVersionValue"
        }
        ...
        require(!provenance.contains("BLOCKS EXTERNAL MODULE RELEASE")) {
            "Official packaging is blocked by the unresolved ownership assignment"
        }
        ...
        require(git("status", "--porcelain").isEmpty()) { "Official releases require a clean worktree" }
...
if (officialRelease.get()) {
    generateCompliance.configure {
        dependsOn(verifyOfficialReleaseInputs)
    }
    tasks.named("signModule") {
        dependsOn(verifyOfficialReleaseInputs)
    }
}

tasks.register<Copy>("packageOfficial") {
    dependsOn(verifyOfficialReleaseInputs, "build")
    from(layout.buildDirectory.file("$moduleFileName.modl"))
    into(rootProject.layout.projectDirectory.dir("release"))
    rename { "$moduleFileName-$moduleVersionValue.modl" }
    doLast {
        val artifact = layout.projectDirectory.file(
            "release/$moduleFileName-$moduleVersionValue.modl"
        ).asFile
        val checksum = MessageDigest.getInstance("SHA-256")
            .digest(artifact.readBytes())
            .joinToString("") { "%02x".format(it) }
